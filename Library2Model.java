...
public class Library2Model implements BooksDbInterface {
   ...
   /* public void executeQuery1(String pass, String uname,
                ...
                while (rs.next()) {
                    ...
                    try{
                        if(!tmpSeenISBNList.contains(tmpBookList.get(0))){
                            ...
                            tmpAuthorList.clear();
                            tmpAuthorList.add(new Author(rs.getString(6),rs.getString(7)));
                            book = new Book(tmpBookList.get(0),bookGenre,tmpBookList.get(2),
                            tmpBookList.get(3),tmpAuthorList,tmpBookList.get(4));
                            this.booklist.add(book);
                            bookCounter++;
                            ItsTreasonThen = true;
                            if(secondLife){
                           ...
                           this.booklist.get(bookCounter).getTheAuthors().add
                            (new Author(rs.getString(6),rs.getString(7)));
    ...
    public String createOrder66(ArrayList<String> usedISBN){
        ...
        Order66 += "select authorof.*,authorname,authorcreator from authorof"
                + " join author on authorof.authorID = author.authorID"
                + " where";
                ...
                while (rs.next()) {
                    ...
                    try{
                       ...
                       if(this.booklist.get(safeRow).getIsbn().equals(rs.getString(1))){      
                            ...
                            rs.getString(4)))){
                                this.booklist.get(safeRow).getTheAuthors().add(
                                new Author(rs.getString(3),rs.getString(4)));
                            }
